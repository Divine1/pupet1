...
//const CSV_LINK = 'http://localhost/config.csv'
const CSV_LINK = 'http://localhost/SampleDepth.csv'
//const CSV_LINK = 'file:///C:/Users/User/Documents/depthurls.xlsx'
// Apify.main() function wraps the crawler logic (it is optional).


var processApify = async (inputParam) => {
    ...
    const requestList = new Apify.RequestList({
        sources: [{ requestsFromUrl: inputParam.CSV_LINK }],
    });
    ...
    const crawler = new Apify.CheerioCrawler({
        ...
        handlePageFunction: async ({ request, html, $ }) => {
            ...
            const title = $(inputParam.title).text();
            const h1texts = [];
            $(inputParam.h1).each((index, el) => {
                h1texts.push({
                    text: $(el).text(),
                });
            });
...
const invokeApify = async (inputParam)=>{

    Apify.main(()=>{
        return processApify(inputParam)
    });
}
